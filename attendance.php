...
let rfidInput = document.getElementById("rfid_input");

// 🔥 AUTO OPEN MODAL
let modal = new bootstrap.Modal(document.getElementById('rfidModal'));
modal.show();
...
// kahit mag click ka anywhere, babalik focus sa scanner
...
// 🔥 pag ENTER na (RFID scanner trigger)
if(e.inputType === "insertLineBreak" || this.value.length >= 10){
    ...
    .then(data => {
        ...
        let photo = document.getElementById("rfid_photo");
        let msg = document.getElementById("rfid_msg");

        if(data.status === "time_in"){
            msg.innerText = "✅ TIME IN: " + data.name;
        }
        else if(data.status === "time_out"){
            msg.innerText = "⏰ TIME OUT: " + data.name;
        }
        else if(data.status === "invalid"){
            msg.innerText = "❌ Unknown Card";
        }
        else{
            msg.innerText = "✔ Already Recorded";
        }

        photo.src = data.photo ? data.photo : "assets/images/default.png";
    ...
    // 🔥 AUTO REFRESH PAGE (PARA UPDATED TABLE)
    location.reload();
