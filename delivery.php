...
if(isset($_POST['update_status'])){
    ...
    if($new_status=="delivered"){
        ...
        /* GET WORK ORDER */
        $stmt=$conn->prepare("
        SELECT wo_no, product_name, qty, selling_price, client_name, client_email
        FROM work_orders
        WHERE id=?
        ");
        $stmt->execute([$wo_id]);
...
try {
    ...
    $mail->setFrom('your_email@gmail.com', 'ENSLP System');
...
SELECT id, wo_no, product_name, qty, selling_price, client_name, client_email
FROM work_orders
...
<option value="<?=$wo['id']?>" data-client="<?=$wo['client_name']?>">
