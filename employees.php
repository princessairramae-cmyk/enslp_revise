...
$stmt=$conn->prepare("
UPDATE employees
SET full_name=?,rfid_uid=?,position=?,department=?,employment_status=?,date_hired=?,salary_type=?,salary_amount=?,contact_no=?
WHERE id=?");

$stmt->execute([$name,$rfid,$pos,$dept,$status,$date,$type,$salary,$con,$id]);
